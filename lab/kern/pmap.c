...
mem_init(void)
{
	...
	cprintf("sizeof Env %d, number of NENV %d \n", sizeof(struct Env), NENV);
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		// Real-mode IDT and BIOS information
		// IO hole and other used memory
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref)
	  panic(" pp->pp_ref is used !");

	if(pp->pp_link)
	  panic(" pp->pp_link is not a page_info address!");		//pay attention
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t *pt;
	struct PageInfo *padd = page_lookup(pgdir, va, &pt);
